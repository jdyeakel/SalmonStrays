...
\begin{align}
  ...
  &+ h^2\sigma\frac{\partial}{\partial \mu_i}\ln\left(w_i R_i[\mu_i(t)|\theta_i] + (1-w_i)R_i[\mu_j(t)|\theta_i]  \right),
  \label{eq:mu}
\end{align}
...
\noindent {\bf (c) Measuring of metapopulation persistence}
...
The average-CV portfolio effect is the average CV across each population divided by the CV of the aggregate, such that
...
A more direct way to measure the likelihood that a system will persist is to measure the time it takes to recover following an induced disturbance: systems that recover quickly (lower recovery times) are more robust than those that recover more slowly (higher recovery times).
Measuring the time it takes for a perturbed system to relax to a steady state also permits a more detailed examination of the potential fragility of the metapopulation.
For example, if populations settle into alternative steady states (alternative steady states in our model requiring one population to be high-density and one low-density), comparing recovery times after a disturbance applied to the high, low, and/or both populations permits an understanding of which component of the metapopulation has a stronger influence on persistence.
\\

\noindent {\bf (d) Density dependent straying rates and the effects of distance}
%Density dependent m
\noindent We have so far assumed that the proportion of strays leaving and entering a population is constant, however there is good evidence that at least in some species the stray rate is density dependent.
...
\noindent{\bf (a) Nonlinear effects of straying on the portfolio effect and return time} \\
...
This bifurcation occurs at lower values of the straying rate $m$ at lower values of trait heritability $h^2$ (Fig \ref{fig:PE}a,b), indicating that weaker coupling between ecological and evolutionary dynamics in addition to higher rates of straying promotes the existence of alternative stable states.

Trait heritability has a large impact on the sensitivity of the straying rate on both the aggregate population steady state density ($N^*_T=N^*_1+N^*_2$) as well as the difference between steady state densities (the distance between alternative stable states: $\Delta N=|N^*_1-N^*_2|$).
Greater trait heritability results in a larger decline in $N_T^*$ for increasing straying rates $m$, though results in only moderate changes to $\Delta N$ (Figs. \ref{fig:PE}A,B).
Conversely, if the trait is has a lower heritability, an increase in the straying rate has little impact on the total biomass density and contrastingly large effects on $\Delta N$ (Fig. \ref{fig:PE}a,b).
...
As the fold bifurcation is reached with greater rates of straying, the portfolio effect explodes due to a large increase in the variance of both donor and recipient populations ${\rm VAR}(N_{i,j})$.
This increase in variance is the product of a dynamical process known as critical slowing down that occurs near fold bifurcations (among others) \cite{Scheffer:2009gg}, and has been suggested to serve as an early warning signal of an approaching phase transition \cite{Scheffer:2009gg,Anonymous:2013br,Dakos:2014br}.
For larger values of $m$ (to the right of the fold bifurcation in Fig \ref{fig:PE}), where there alternative steady states among the recipient and donor sites, the portfolio effect declines steadily after the explosion in variance associated with onset of the fold bifurcation.
...
We measured the rate of recovery to a steady state as a function of the rate of straying $m$ following three types of induced disturbance: (\emph{i}) extinction of the low-density population, (\emph{ii}) extinction of the high-density population; (\emph{iii}) near-collapse of both populations where just 1.0\% of both populations survive.
At low rates of straying both populations are subject to the same steady state, such that extinction of either population results in the same recovery time, which is generally less than that required after a near-collapse of both populations.
For values of $m$ that give rise to alternative steady states, the recovery times associated with each disturbance type depends strongly on trait heritability.
If both straying rate and heritability are low (Fig. \ref{fig:relax}a), extinction of the small population results in a lower recovery time, whereas as the rate of straying increases, the extinction of the larger population results is lower recovery times.
...
For straying rates in the alternative steady state regime, extinction of the smaller population consistently provides the least time to recovery.
