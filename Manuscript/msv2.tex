...
As the portfolio effect is highly sensitive to the rate of straying between populations, so is the time the system requires to recover following a large disturbance.
We measured the time required for the system $N_T$ to relax to its steady state following three types of induced disturbance: (\emph{i}) extinction of the low-density population; (\emph{ii}) extinction of the high-density population (scenarios \emph{i} and \emph{ii} are equivalent in the single steady state regime); (\emph{iii}) near-collapse of both populations where just 1.0\% of both populations survive.
In general, the average-CV portfolio effect is negatively correlated with recovery time (Fig. \ref{fig:PE}d), indicating that for our system both measures are valuable indicators of metapopulation persistence.
...
This switch is in part due to a large burst in biomass of the surviving (large) population when the smaller of the two becomes extinct at high straying rates, which extends the time required for the smaller population (and thus the system as a whole) to recover.
Increased trait heritability flips this relationship (Fig. \ref{fig:relax}b), such that systems suffering near-collapse of both populations recover more quickly than if either population is lost at low straying rates.
As straying rates increase, the system recovers more quickly if the small population is lost, whereas the time to recovery is maximized if both populations suffer near-collapse.
Near the onset of the fold bifurcation, recovery times increase drastically regardless of straying rate, heritability, or disturbance type.
...
Less intuitively, we observe a discrete jump towards low phenotypic diversity as the fold bifurcation is crossed (Fig. \ref{fig:traitdiff}).
...
With the onset of straying, we find that increasingly divergent trait optima generally lower $N_T$ and exaggerate $\Delta N$, such that the biomass distribution becomes increasingly uneven.
...
As habitat heterogeneity increases, the fold bifurcation occurs at lower straying rates whereas the associated jump in the PE is greater (Fig. \ref{fig:thetaPE}), decreasing the recovery time (Appendix Fig. \ref{fig:thetarelax}).
For greater straying rates, habitat heterogeneity erodes the PE and slows recovery time (Fig. \ref{fig:thetaPE}, Appendix Fig. \ref{fig:thetarelax})
...
%This means that alternative stable states can emerge with relatively little dispersal between sites (Fig. \ref{fig:thetaPE}), however the increased PE (and shorter recovery times).
\\
...
For instance, the recovery times for systems with constant and density dependent straying both show similar trends for a given $m$ or $m^*$ (Fig. \ref{fig:relax}c,d), however there are some distinct quantitative differences.
When straying rate and trait heritability are low, near-collapse of both populations result in longer than expected recovery times, whereas the recovery times in the alternative stable state regime (higher $m^*$) are similar (Fig. \ref{fig:relax}c).
...
We find that the resultant portfolio effect approximates that generated by a system with a similar constant straying rate, such that ${\rm PE}(m) \approx {\rm PE}(m^*_{\rm high-density})$ (Fig. \ref{fig:thetaPE}b).
As the steady state straying rate increases, the portfolio effect follows a similar trajectory as shown for metapopulations with constant straying rates (Fig. \ref{fig:thetaPE}a), though is generally higher, promoting persistence. 
RETURN TIMES
...
% 
% We find that the by-definition lower straying rate of the high-density population (left-most of the paired points in Fig. \ref{fig:thetaPE}b) largely determines the portfolio effect for the metapopulation.
% In other words, 
\\
...
\centering
\includegraphics[width=0.5\textwidth]{figs2/fig_relaxtheta.pdf}
\caption{
